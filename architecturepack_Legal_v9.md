# Architecture Pack - Legal Workflow System v9

> Project: **Legal**
> Date: 2026-04-04
> Version: v9 - Cloud SQL, Notification System, Dashboard/MyTasks/Upload UX improvements
...
| # | Update | Type | Sections | Tóm tắt |
|---|--------|------|----------|----------|
| 1 | Cloud SQL PostgreSQL | INFRASTRUCTURE | S6.5, S7.6 | Database persistent, data khong mat khi redeploy. Dual-backend (PG production, SQLite dev) |
| 2 | Document Link URL | FEATURE | S1.1, S3 | Them  vao TDI. View mo external link (GG Slides/Docs), Download tai file |
| 3 | Login Persistence | BUGFIX | S3, S6 | Token luu localStorage, JWT expiry check, auto-logout on 401 |
| 4 | BigQuery Auth Production | BUGFIX | S1.3 | USE_BIGQUERY_AUTH=true, 459 user @apero.vn login duoc. Fix MockPermissionService -> CombinedPermissionService |
| 5 | Dashboard Clickable + Status Logic | FEATURE + BUGFIX | S3.3, S6 | Click status card -> filter My Tasks. Dem L1 root tasks + latest_status (khong dem L2/L3) |
| 6 | Create Task Droplist | FEATURE | S3.8 | Datalist suggestions cho PT, LE, CDT, CTY, TUT, TLT filters |
| 7 | My Tasks UI Improvements | FEATURE | S3.4 | Header highlight xanh, cot Assigned To + Submitted By hien id_name (batch BigQuery lookup) |
| 8 | Upload Cloud CTA | UI | S3.5 | Cloud icon upload, border dashed, nut do noi bat. Thong nhat giao dien cho tat ca roles |
| 9 | Unified Actions (All Roles) | FEATURE | S3.5 | Approver/Checker/Submitter cung giao dien: Upload + AI Check + Submit + Approve/Reject |
| 10 | Step Order Logic | BUGFIX | S3.5, S5 | Step chi editable khi tat ca step truoc da done (APPROVED/REJECTED/COMPLETED) |
| 11 | Download Fix | BUGFIX | S3.5 | Dung ten file goc khi download. <a download> tag thay vi fetch blob |
| 12 | Name Lookup Optimization | PERFORMANCE | S6 | Batch BigQuery 1 lan thay vi N lan. Giam tu 30s xuong <3s |
| 13 | Email Notification System | FEATURE | S3.5, S5, S6 | Send Back to Submitter (modal preview) + Notify Reviewer + Notify on Assign. Gmail SMTP |
| 14 | Exception Users | CONFIG | S1.3 | Them legal@apero.vn (Submitters), fix nguyenvietrung187@gmail.com |
| 15 | ProcessDescription Enrichment | DOC | S2 | Bo sung khai niem, dieu kien TNT, FORK/JOIN, LOOP BACK, ESCALATE |
| 16 | Unified Actions UI | FEATURE | S3.5 | Approver/Checker/Submitter cung giao dien Actions. Submitter read-only Progress Tree |
| 17 | Step Order Fix | BUGFIX | S3.5, S5 | Steps hien dung thu tu (ORDER BY created_at). isEditable yeu cau isAdmin |
| 18 | Document Preview | FEATURE | S3.5 | Collapsible dropdown selector, preview PDF/IMG inline (600px) |
| 19 | Upload PE Evidence | FEATURE | S3.5 | Nut Upload PE canh Ma PE (PDF/JPG/PNG). LF240 only |
| 20 | Create Task Cleanup | UI | S3.8 | Bo Department, Priority, Upload. Auto deadline T+1/T+2. LF240 user chon due date |
| 21 | Assigned To from BigQuery | FEATURE | S3.5, S6 | Dropdown chi hien Checker + Approver tu BigQuery v_auth_lookup |
| 22 | Submitter Submit Fix | BUGFIX | S5, S6 | Task creator (requested_by) duoc phep Submit to Review |
| 23 | TrangCTH -> Approver | CONFIG | S1.3 | Update role_legal trong BigQuery fps_data.emp_list |
...
**Ghi chu**: TST co cau truc phan cap 3 level + nhom theo Product:
- **Product (L0)**: San pham Apero. Nhom logic dua tren `TSI_Filter.filter_type=PT`. Moi Product co nhieu task L1.
- **Level 1** (Loai Task): `Copyright`, `Trademark`, `Policy`, `Contract`
...
| created_at | TIMESTAMP | Y | Ngay tao |
| updated_at | TIMESTAMP | Y | Ngay cap nhat |
| metadata | STRING (JSON) | N | Du lieu bo sung theo loai task (vd: LF240 chua requester_phone, manager_email, partner info, purpose, pe_code) |
...
| file_url | STRING | Y | URL luu tru: Google Cloud Storage (`gs://legal-workflow-docs/{tsi_id}/{uuid}.ext`) hoac signed URL. Legacy: local `/api/legal/task/{tsi_id}/file/{name}` |
...
| link_url | STRING | N | External URL cho View (Google Slides, Docs, etc.) — v8 |
| storage_type | STRING | N | Noi luu tru: `GCS` (Google Cloud Storage) hoac `LOCAL` (server filesystem). Default: `GCS` |
...
| event_type | STRING | Y | Loai su kien: `CREATE`, `UPLOAD`, `VIEW`, `UPDATE`, `COMMENT`, `APPROVE`, `REJECT`, `REASSIGN`. emp_id=`AI_REVIEWER` cho AI auto-review |
...
| google_email | STRING | N | Email Google SSO (vd: `tiepta@apero.vn`). Sinh tu: `LOWER(id_name) + "@apero.vn"` |
| empsec | STRING | N | Cap bao mat: `SEC1`, `SEC2`, `SEC3`, `SEC4`. Tu dong tinh tu empgrade |
| pt_allowed | STRING | N | Quyen xem Product Type: `MyPT` hoac `AllPT` |
| cdt_allowed | STRING | N | Quyen xem CDT: `MyCDT`, `MyCDTParent`, `AllCDT` |
| krf_level | INT | N | Do sau xem du lieu: 3 (PL3) hoac 7 (PL7) |
| cdt_1 | STRING | N | Ma cong ty goc (vd: `HQ1`, `AST`, `SAP`) |
...
## 1.3 SEC Permission System (PD250 SECURITY)

> **Nguon**: Tich hop tu `SEC_Permission_System.md`. Dataset: `fp-a-project.sec_data`
...
| `cdt` | CDT path đầy đủ |
| `role_legal` | Submitters / Checker / Approver |
...
### 1.3.6 Tích hợp Google SSO (Option A — Application Layer)
...
### 1.3.7 Logic phân quyền theo cấp
...
## 1.4 Relationship Map (Workflow Entity ONLY)
...
## 1.5 Entity Relationship Diagram (Text-based)
...
### 2.1.1 Mo hinh quan ly: Product -> Legal Process

Moi san pham (Product) co nhieu quy trinh Legal chay dong thoi.

```
Product (L0 - San pham)
+-- LF210 - Copyright Check (L1)
+-- LF220 - Trademark Check (L1)  
+-- LF230 - Policy PP & TOS (L1)
+-- LF240 - Contract Review (L1)
```

### 2.1.2 Bang tong hop quy trinh
...
### 2.1.3 Cac khai niem chinh

| Khai niem | Giai thich |
|-----------|------------|
| **TST** | Dinh nghia loai task. L1 (Loai) -> L2 (Phase) -> L3 (Step) |
| **TSI** | 1 lan thuc hien cu the. Vd: Copyright Check cho app Music v2.1 |
| **TNT** | Quy tac chuyen buoc: tu TST nao -> den TST nao, dieu kien gi |
| **TDI** | Tai lieu dinh kem (file upload + link). View mo link, Download tai file |
| **TSEV** | Nhat ky: CREATE, UPLOAD, APPROVE, REJECT, REASSIGN, COMMENT |
| **TRI** | Phan cong: ai lam task nao |
| **SLA** | Thoi gian toi da. T+2 = 2 ngay lam viec |
| **Product (L0)** | San pham Apero. Nhom task Legal theo san pham |

### 2.1.4 Trang thai task (Status Flow)

```
DRAFT -> IN_PROGRESS -> PENDING_REVIEW -> SUBMITTED -> APPROVED -> COMPLETED
                    |-> REJECTED (chinh sua lai)
                    |-> CANCELLED (huy)
```

| Trang thai | Y nghia | Ai thao tac |
|------------|---------|-------------|
| DRAFT | Vua tao | System |
| IN_PROGRESS | Dang xu ly | Assignee |
| PENDING_REVIEW | Cho review | System |
| SUBMITTED | Checker xac nhan | Checker |
| APPROVED | Approver phe duyet | Approver |
| REJECTED | Tu choi, can sua | Approver/Checker |
| COMPLETED | Hoan thanh | System |
| CANCELLED | Huy bo | Admin |

---

## 2.2 Process: LF210 - Copyright Check

> **v8 Change — LF210 Create Task:**
> - **Bo**: Urgency (TUT), Legal Entity (LE), Country (CTY) khoi form tao task
> - **Them**: CDT filter (Department), Product Name (PT_NAME)
> - **Bat buoc (*)**: Product Code (PT), Product Name (PT_NAME), Title
> - **Them**: Nut "Upload" cho phep upload file truc tiep khi tao task
...
| From TST | To TST | Condition | Mo ta |
|----------|--------|-----------|-------|
| LF211 | LF212 | `has_document(tdt_code='UI_COMPARISON_FILE') >= 1` | Product/ASO upload it nhat 1 file so sanh UI/UX. Neu chua -> block |
| LF212.1 | LF212.2 | `auto_sequential(prev_status='APPROVED')` | Doi chieu man hinh APPROVE -> tu dong chuyen Kiem tra Asset |
| LF212.2 | LF212.3 | `auto_sequential(prev_status='APPROVED')` | Kiem tra Asset APPROVE -> chuyen Kiem tra AI Image |
| LF212.3 | LF212.4 | `auto_sequential(prev_status='APPROVED')` | Kiem tra AI APPROVE -> chuyen Doi chieu Store |
| LF212.4 | LF212.5 | `auto_sequential(prev_status='APPROVED')` | Doi chieu Store APPROVE -> chuyen Tong hop |
| LF212.5 | (END) | `review_result IN ('PASS','CAN_SUA','FAIL')` | PASS (dat) / CAN SUA (vi pham nhe) / FAIL (nghiem trong). Comment bat buoc khi CAN_SUA/FAIL |
...
| From TST | To TST | Condition | Mo ta |
|----------|--------|-----------|-------|
| LF221 | LF222 | `has_document(tdt_code='APP_INFO') >= 1` | Product gui du: ten app, icon, graphic, description, keywords |
| LF222 | LF222.1-6 | `fork(type='parallel', create_count=6)` | **FORK**: Tao 6 TSI L3 dong thoi. 6 buoc tra cuu KHONG phu thuoc nhau |
| LF222.1 | (wait) | `search_complete(source='WIPO')` | Tra cuu WIPO Global Brand Database |
| LF222.2 | (wait) | `search_complete(source='USPTO')` | Tra cuu USPTO TESS |
| LF222.3 | (wait) | `search_complete(source='SHTT_VN')` | Tra cuu Cuc SHTT Viet Nam |
| LF222.4 | (wait) | `search_complete(source='GOOGLE')` | Tra cuu Google Search ten app + keywords |
| LF222.5 | (wait) | `search_complete(source='WIPO_IMAGE')` | Tra cuu WIPO Image Database (icon) |
| LF222.6 | (wait) | `search_complete(source='GOOGLE_REVERSE_IMG')` | Tra cuu Google Reverse Image (icon) |
| LF222.1-6 | LF222.7 | `join(type='all', check='COUNT(COMPLETED)==6')` | **JOIN**: 6/6 COMPLETED moi unlock. 1 chua xong -> block |
| LF222.7 | (END) | `review_result IN ('PASS','CAN_SUA','FAIL')` | PASS (an toan) / CAN SUA (trung 1 phan) / FAIL (da dang ky) |
...
├── LF234-Ban noti cho ASO (L2, PIC: ASO, SLA: T+3)
```
...
| From TST | To TST | Condition | Mo ta |
|----------|--------|-----------|-------|
| LF231 | LF232 | `has_document('PERMISSION_FORM') AND form_valid=true` | Product+Dev dien du form permission & data collection |
| LF232.1 | LF232.2 | `legal_review(form_validation='PASS')` | Legal kiem tra form hop le -> chuyen buoc |
| LF232.1 | LF231 | `legal_review(form_validation='FAIL')` | **LOOP BACK**: Thieu/sai -> REJECT + comment -> tra lai Product |
| LF232.2 | LF232.3 | `auto_sequential(prev_status='APPROVED')` | Doi chieu phap ly xong -> soan PP |
| LF232.3 | LF232.4 | `fork(type='parallel', tasks=['PP','TOS'])` | **FORK**: Soan PP va TOS dong thoi |
| LF232.3+4 | LF232.5 | `join(type='all', check='PP_done AND TOS_done')` | **JOIN**: PP+TOS xong moi gui draft |
| LF232.5 | LF233 | `auto_sequential` | Gui draft PP+TOS cho Product review |
| LF233 | LF234 | `product_review(result='APPROVED')` | Product dong y -> ban noti cho ASO |
| LF233 | LF232 | `product_review(result='REVISION_REQUESTED')` | **LOOP BACK**: Yeu cau sua -> REJECT -> Legal sua. round_count+=1 |
| LF234 | (END) | `notify_aso(sent=true, channel='email+slack')` | Legal gui noti cho ASO de dang PP/TOS len Store. Task COMPLETED khi ASO xac nhan |
...
> **v8 Change — LF240 IN_PROGRESS Additional Fields:**
> Khi task LF240 o trang thai IN_PROGRESS, hien thi them cac truong input duoi Progress Tree:
>
> | Truong | Key | Bat buoc | Type |
> |--------|-----|----------|------|
> | SDT cua nguoi de xuat | `requester_phone` | * | STRING |
> | Email cua quan ly nguoi de xuat | `manager_email` | * | STRING (email) |
> | SDT doi tac | `partner_phone` | | STRING |
> | Mail lien he cua doi tac | `partner_contact_email` | * | STRING (email) |
> | Mail ky hop dong cua doi tac | `partner_contract_email` | | STRING (email) |
> | Muc dich ky ket/ ban hanh | `purpose` | * | STRING (textarea) |
> | Ma PE | `pe_code` | * | STRING |
>
> Du lieu luu trong `TSI.metadata` (JSON). Cac truong * phai dien truoc khi chuyen phase tiep theo.
...
├── LF245-Luu tru & ma hoa HD (L2, PIC: Legal + Hanh chinh)
│   └── Day chuyen: Bo phan de xuat -> Legal -> Ke toan -> Tro ly GD -> BOD -> CEO -> Hanh chinh
├── 
├── LF246-Theo doi & nhac gia han (L2, PIC: Legal, Dinh ky hang thang)
```
...
| From TST | To TST | Condition | Mo ta |
|----------|--------|-----------|-------|
| LF241 | LF242 | `has_document('CONTRACT_DRAFT') AND metadata_required_valid=true` | Bo phan gui HD draft + metadata (SDT, email, PE code). Thieu -> block |
| LF242 | LF243 | `legal_review(input_validation='PASS')` | Legal xac nhan du thong tin, HD hop le |
| LF242 | LF241 | `legal_review(input_validation='FAIL')` | **LOOP BACK**: Thieu/sai -> REJECT + comment -> tra lai bo sung |
| LF243.1 | LF243.2 | `fork(type='parallel', tasks=['LEGAL','FINANCE'])` | **FORK**: Legal + Ke toan ra soat DONG THOI |
| LF243.1+2 | LF243.3 | `join(type='all', check='legal_done AND finance_done')` | **JOIN**: Ca 2 xong moi tong hop feedback |
| LF243.3 | LF243.4 | `feedback(has_revision_items=true)` | Co diem chinh sua -> gui feedback + dam phan |
| LF243.3 | LF244 | `feedback(has_revision_items=false)` | HD sach -> chuyen thang Final Check |
| LF243.4 | LF243.3 | `negotiation(round_count<3 AND agreement=false)` | **NEGOTIATION LOOP**: Doi tac sua -> review lai. round_count+=1. Max 3 vong |
| LF243.4 | LF244 | `negotiation(agreement=true)` | Thong nhat -> Final Check |
| LF243.4 | ESCALATE | `negotiation(round_count>=3 AND agreement=false)` | **ESCALATE**: >3 vong -> escalated=true -> trinh BOD |
| LF244 | LF245 | `legal_final_check(result='PASS')` | HD phien ban cuoi hop le -> luu tru & ma hoa |
| LF245 | LF246 | `archive(coded=true, format='ddmmyy/Cty-DoiTac')` | HD da ky -> ma hoa (vd: 230326/OMC-Apero-TikTok) + luu kho |
| LF246 | (END) | `monitor(status IN ('EXPIRED','TERMINATED'))` | HD het han / cham dut -> COMPLETED |

| LF246 | LF240 (new) | `monitor(renewal=true, days_before_expiry<=30)` | Gia han: nhac 30 ngay truoc. Tao LF240 moi + ref HD cu |
...
## 3.4 WF02 - My Tasks (Nhom theo San pham - v8)
...
- **v8**: Nhom theo Product (PT filter) truoc, sau do so ra task L1
- Click Product card -> so ra danh sach task L1 cua san pham do
- Click [View] -> vao Task Detail (WF03)
- Admin (Approver/Checker/SEC4): nhin tat ca task
- Submitters: chi nhin task cua minh (requested_by hoac assigned_to)
...
│  │ Submitters:  Read-only status badges                        ││
│  └──────────────────────────────────────────────────────┘│
│                                                           │
...
│  │ Submitters:  [Upload Document] [AI Check] [Submit to Review] │ │
│  │        + [Upload Document]                           │ │
...
- Submitters chi co 2 nut: Upload Document + Submit
...
## 3.8 Create Task — LF210 Copyright Check (v8)

```
┌──────────────────────────────────────────────────────────┐
│  Create Task — Copyright Check (LF210)                    │
├──────────────────────────────────────────────────────────┤
│  Task Type: [Copyright Check (LF210) ▼]                   │
│  Title *: [____________________________________]          │
│  Priority: [MEDIUM ▼]                                     │
│  Due Date: [____/____/____]                               │
│                                                           │
│  ─── Filters ─────────────────────────────────────────── │
│  Product Code *:  [________] (autocomplete from PT list)  │
│  Product Name *:  [________] (auto-fill khi chon PT)      │
│  Department (CDT): [________] (dropdown, loc theo SEC)    │
│                                                           │
│  ─── Upload ──────────────────────────────────────────── │
│  [Upload tai lieu tu may tinh]                             │
│                                                           │
│  [Create Task]                                            │
└──────────────────────────────────────────────────────────┘
```

> **So voi v7:** Bo Urgency, Legal Entity, Country. Them CDT. Product Code/Name bat buoc (*). Them nut Upload.

## 3.9 Task Detail — Layout v8

```
┌──────────────────────────────────────────────────────────┐
│  [← Back to Tasks]                                        │
│                                                           │
│  Task: aaaa                                [IN_PROGRESS]  │
│  TSI-20260330-001                                         │
│  Priority: MEDIUM | Due: - | Created: 30/3/2026           │
│  Assigned To: [▼ TiepTA - Legal Manager ] [Reassign]     │ ← v8: Dropdown
│  Tags: [PT: APB648] [CDT: HQ1]                           │
│                                                           │
├──── Progress Tree ───────────────────────────────────────────┤
│  (giu nguyen v7)                                          │
│                                                           │
├──── LF240 Additional Fields (chi LF240 IN_PROGRESS) ────┤ ← v8 moi
│  SDT nguoi de xuat *:    [+84-___-___-___]               │
│  Email quan ly *:         [________@apero.vn]             │
│  SDT doi tac:            [___-___-___]                    │
│  Mail lien he doi tac *: [________@____.__]               │
│  Mail ky HD doi tac:     [________@____.__]               │
│  Muc dich ky ket *:      [________________________]       │
│  Ma PE *:                 [PE-____-___]                    │
...
│                                                           │
├──── Actions ─────────────────────────────────────────────────┤
│  (giu nguyen v7: Upload, AI Check, Submit, Approve/Reject)│
│                                                           │
├──── Event Log (collapsible, mac dinh dong) ──────────────┤ ← v8: Dua xuong cuoi
│  (giu nguyen v7)                                          │
└──────────────────────────────────────────────────────────┘
```

> **v8 Layout order:** Progress Tree → LF240 Fields → Documents → Actions → Event Log (cuoi cung)
> **So voi v7:** Event Log tu giua Documents-Actions → xuong duoi cung. Them Reassign dropdown. Them LF240 fields.
...
## 4.3 UserFlow: Reassign Task (v8)

```
Admin/Approver mo TaskDetail
       ↓
Click dropdown "Assigned To"
       ↓
He thong load danh sach EMP co TRT phu hop voi TST hien tai
       ↓
Chon employee moi → Click [Reassign]
       ↓
API: PUT /api/legal/task/{tsi_id}/reassign
       ↓
Tao TSEV event (type=REASSIGN, data={old_emp, new_emp, reason})
       ↓
Cap nhat TSI.assigned_to = new_emp
       ↓
Thong bao cho ca nguoi cu va nguoi moi
```

**Quyen reassign:**
- SEC4 / Approver: Duoc reassign bat ky task nao
- SEC3 / Manager: Duoc reassign task trong pham vi CDT cua minh
- SEC1-2 / Submitters: Khong duoc reassign

---

## 4.4 UserFlow 3: Reports
...
## 5.7 Contract Expiry Monitoring (LF246)
...
## 5.8 File Upload & Storage (v8 — Google Cloud Storage)

> **v8 Change:** File luu tru chuyen tu local filesystem sang Google Cloud Storage (GCS) de dam bao persistent storage tren Cloud Run.

### GCS Bucket Structure
```
gs://legal-workflow-docs/
├── {tsi_id}/
│   ├── {uuid1}.pdf
│   ├── {uuid2}.docx
│   └── {uuid3}.xlsx
...
GCS_BUCKET = "legal-workflow-docs"

async def upload_to_gcs(tsi_id: str, file_content: bytes, ext: str) -> str:
    """Upload file len Google Cloud Storage."""
    client = storage.Client()
    bucket = client.bucket(GCS_BUCKET)
    blob_name = f"{tsi_id}/{uuid4().hex[:8]}{ext}"
    blob = bucket.blob(blob_name)
    blob.upload_from_string(file_content)
    return f"gs://{GCS_BUCKET}/{blob_name}"

def get_signed_url(gcs_uri: str, expiration_minutes: int = 60) -> str:
    """Tao signed URL de download file."""
    client = storage.Client()
    bucket = client.bucket(GCS_BUCKET)
    blob = bucket.blob(gcs_uri.replace(f"gs://{GCS_BUCKET}/", ""))
    return blob.generate_signed_url(expiration=timedelta(minutes=expiration_minutes))
```

---

## 5.9 AI Review Logic (GPT-4o-mini)

```python
# Trigger: Submitters nhan "AI Check" hoac tu dong sau "Submit to Review"
...
**File upload flow**:
```
Submitters chon file -> POST /upload-file (multipart) -> Luu uploads/{tsi_id}/{uuid}.ext
                                                 -> TDI record (file_url = /api/.../file/{name})
                                                 -> TSEV UPLOAD event
```

## 5.10 Task Reassignment Logic (v8)

```python
def reassign_task(tsi_id: str, new_emp_code: str, reason: str, current_user: dict):
    """Chuyen task cho nguoi khac."""
    tsi = tsi_repository.get_by_id(tsi_id)
    old_emp = tsi.assigned_to

    # Validate quyen reassign
    if current_user["empsec"] not in ("SEC3", "SEC4") and current_user["role_legal"] != "Approver":
        raise PermissionError("Khong co quyen reassign")

    # Validate new_emp co TRT phu hop
    tst = tst_repository.get_by_id(tsi.tst_id)
    required_trts = tst_trt_repository.get_by_tst(tst.tst_id)
    # ... kiem tra new_emp co trong TRI base pool
...
## 5.11 LF240 Metadata Validation (v8)

```python
REQUIRED_LF240_FIELDS = [
    "requester_phone", "manager_email",
    "partner_contact_email", "purpose", "pe_code"
]

def validate_lf240_metadata(tsi, metadata: dict) -> bool:
    """Kiem tra cac truong bat buoc cua LF240 truoc khi chuyen phase."""
    if not tsi.tst_id.startswith("LF24"):
        return True

    missing = [f for f in REQUIRED_LF240_FIELDS if not metadata.get(f)]
    if missing:
        raise ValueError(f"Thieu truong bat buoc: {', '.join(missing)}")

    # Validate email format
    for email_field in ["manager_email", "partner_contact_email", "partner_contract_email"]:
        val = metadata.get(email_field, "")
        if val and "@" not in val:
            raise ValueError(f"Email khong hop le: {email_field}")
    return True
```
...
## 5.12 Email Notification System (v9)

### Trigger Events

| Trigger | Sender | Receiver | Email Subject |
|---------|--------|----------|---------------|
| Send Back to Submitter | Approver | Submitter (requested_by) | [Legal] Feedback: task_code - title |
| Notify Reviewer | Submitter | Approver (assigned_to) | [Legal] Review needed: task_code |
| Reassign Task | Approver | Nguoi duoc assign | [Legal] Task assigned to you |

### SMTP: Gmail App Password (trungnv1@apero.vn)

### Email Lookup: EXCEPTION_USERS -> Mock -> BigQuery v_auth_lookup

### Files: notification/service.py, notification/router.py, tsi/router.py

---

## 5.13 Performance Optimization (v9)

### Name Lookup: Batch BigQuery (1 query thay vi N queries)

| Metric | Truoc (v8) | Sau (v9) |
|--------|------------|----------|
| Dashboard load | 5-10s | <2s |
| My Tasks load | 20-30s | <3s |
| Name lookup | N BigQuery queries | 1 batch query + cache |
...
| Trigger | Sender | Receiver | Email Subject |
|---------|--------|----------|---------------|
| Approver/Checker click "Send Back to Submitter" | Approver | Submitter (requested_by) | [Legal] Feedback: {task_code} - {title} |
| Submitter click "Notify Reviewer" | Submitter | Approver (assigned_to) | [Legal] Review needed: {task_code} - {title} |
| Approver reassign task | Approver | Nguoi duoc assign moi | [Legal] Task assigned to you: {task_code} - {title} |

### Send Back Flow (Approver -> Submitter)



### Notify Reviewer Flow (Submitter -> Approver)



### Email Lookup Priority
...
| SMTP_PORT | 587 |
| SMTP_USER | trungnv1@apero.vn |
| SMTP_FROM | trungnv1@apero.vn |
| SMTP_PASS | (App Password - env var only) |

### Files

| File | Mo ta |
|------|-------|
|  | send_email(), notify_send_back(), notify_submitted(), notify_assigned() |
|  | POST /send-back, POST /notify-reviewer, _lookup_email() |
|  | Notify on reassign (goi notify_assigned) |
...
### Name Lookup: N+1 Problem -> Batch Query

**Truoc (v8):** Moi task goi 1 BigQuery query de lookup ten -> 10 tasks = 10 queries = 30s

**Sau (v9):** Batch load 1 lan cho tat ca codes:



**Ket qua:** 10 tasks tu 30s -> <3s (giam 90%)

### Dashboard Optimization

Dashboard chi dem L1 root tasks (khong dem L2/L3 sub-tasks). Dung  lay status cua L3 moi nhat.

| Metric | Truoc | Sau |
|--------|-------|-----|
| Dashboard load | 5-10s | <2s |
| My Tasks load | 20-30s | <3s |
| Name lookup | N queries | 1 batch query |
...
| F26 | SEC Permission System | Security | All | P0 |
| F27 | Google SSO Login | Auth | All | P0 |
| F28 | PT/CDT Data Filtering | Security | All | P0 |
| F29 | Task Reassignment (Droplist) | Operation | All | P1 |
| F30 | GCS Document Storage | Infrastructure | All | P0 |
| F31 | LF240 Additional Metadata | Domain | LF240 | P0 |
...
|--------|----------|-------------|------|
| POST | `/api/auth/login` | Login bang Google token hoac email, tra JWT voi SEC fields | Public |
| GET | `/api/auth/me` | Lay thong tin SEC cua user hien tai | JWT |

### Task Reassignment APIs (v8)

| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| PUT | `/api/legal/task/{tsi_id}/reassign` | Chuyen task cho employee khac | SEC3+ |
| GET | `/api/legal/emp/assignable?tst_id={tst_id}` | Danh sach EMP co the assign cho loai task | JWT |

### LF240 Metadata APIs (v8)

| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| PUT | `/api/legal/task/{tsi_id}/metadata` | Cap nhat metadata (LF240 fields) | JWT |
| GET | `/api/legal/task/{tsi_id}/metadata` | Lay metadata | JWT |

### File Storage APIs (v8 — GCS)

| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| POST | `/api/legal/task/{tsi_id}/upload-file` | Upload file len GCS | JWT |
| GET | `/api/legal/task/{tsi_id}/file/{filename}` | Tao signed URL de download | JWT |
...
|--------|------|------|-------|
| POST | /api/legal/task/{id}/send-back | JWT | Approver gui feedback cho Submitter (email + event log) |
| POST | /api/legal/task/{id}/notify-reviewer | JWT | Submitter thong bao Reviewer co task can review |
| POST | /api/auth/login | Public | Login bang Google SSO hoac email (BigQuery + Exception Users) |
...
> **v8 Update**: Database chuyen tu SQLite (ephemeral tren Cloud Run) sang **Cloud SQL PostgreSQL** (persistent).
> - Instance: `legal-workflow-db` (db-f1-micro, asia-southeast1)
> - Database: `legal_workflow`
> - Connection: Cloud Run -> Cloud SQL Auth Proxy (Unix socket)
> - Dual-backend: PostgreSQL (production), SQLite (local dev/test)
> - Toan bo 21 tables tuong thich — wrapper tu dong translate SQL syntax
---
...
| `expiry_date` | DATE | Ngay het han (cho HD - dung cho LF246) |
...
## 7.5 Migration: Local Storage → Google Cloud Storage (v8)

**Phase 1 — Dual Write (hien tai):**
- Upload moi di vao GCS (`gs://legal-workflow-docs/`)
- Env var `STORAGE_BACKEND=gcs` de bat
- Fallback: neu GCS unavailable → luu local

**Phase 2 — Migration:**
- Script doc thu muc `uploads/`, upload tung file len GCS
- Cap nhat `TDI.file_url` tu local path sang GCS URI
- Cap nhat `TDI.storage_type` = `GCS`

**Phase 3 — Cleanup:**
- Xoa file local sau khi verify GCS day du
- Bo endpoint serve file local (hoac giu lam fallback)

---

## 7.6 Migration: SQLite → Cloud SQL PostgreSQL (v8)

### 7.6.1 Van de
- SQLite la file-based DB, ephemeral tren Cloud Run (mat data khi redeploy)
- Khong ho tro multi-instance (moi instance co DB rieng)
- Khong co backup, recovery, failover

### 7.6.2 Giai phap: Cloud SQL PostgreSQL
- **Instance**: `legal-workflow-db` (PostgreSQL 15, db-f1-micro, asia-southeast1)
- **Chi phi**: ~$7/thang (HDD 10GB)
- **Connection**: Cloud Run Service Account + Cloud SQL Auth Proxy (Unix socket)
- **Env var**: `DATABASE_URL=postgresql://legal_app:<PW>@/legal_workflow?host=/cloudsql/fp-a-project:asia-southeast1:legal-workflow-db`

### 7.6.3 Dual-Backend Architecture
```
Production (Cloud Run):
  DATABASE_URL set → PGConnectionWrapper → PostgreSQL
  
Local Development:
  DATABASE_URL empty → sqlite3 (giu nguyen workflow hien tai)
```

**PGConnectionWrapper** tu dong translate:
| SQLite | PostgreSQL |
|--------|------------|
| `?` placeholder | `%s` |
| `:name` placeholder | `%(name)s` |
| `INSERT OR IGNORE` | `INSERT ... ON CONFLICT DO NOTHING` |
| `sqlite3.Row` (dict access) | `psycopg2.extras.RealDictCursor` |
| `AUTOINCREMENT` | `SERIAL` |
| `last_insert_rowid()` | `RETURNING id` |

### 7.6.4 Files thay doi
| File | Thay doi |
|------|----------|
| `src/config/database.py` | Rewrite: dual-backend + PGConnectionWrapper |
| `src/config/settings.py` | +1 dong: `DATABASE_URL` |
| `requirements.txt` | +1 dong: `psycopg2-binary==2.9.9` |
| `src/modules/tsi_filter/repository.py` | Fix `last_insert_rowid()` → `RETURNING id` |

> **Tong: 4 files thay doi, 13+ files khong can sua** nho wrapper tu dong translate.

### 7.6.5 GCP Setup
```bash
# 1. Tao Cloud SQL instance
gcloud sql instances create legal-workflow-db   --database-version=POSTGRES_15 --tier=db-f1-micro   --region=asia-southeast1 --storage-type=HDD --storage-size=10GB

# 2. Tao database va user
gcloud sql databases create legal_workflow --instance=legal-workflow-db
gcloud sql users create legal_app --instance=legal-workflow-db --password=<PW>

# 3. Grant IAM role cho Cloud Run SA
gcloud projects add-iam-policy-binding fp-a-project   --member="serviceAccount:<CR_SA>" --role="roles/cloudsql.client"

# 4. Deploy voi Cloud SQL
gcloud run deploy lww-backend --source . --region=asia-southeast1   --add-cloudsql-instances=fp-a-project:asia-southeast1:legal-workflow-db   --set-env-vars="DATABASE_URL=postgresql://legal_app:<PW>@/legal_workflow?host=/cloudsql/fp-a-project:asia-southeast1:legal-workflow-db"
```

### 7.6.6 Data Persistence Map
| Data | Truoc (SQLite) | Sau (Cloud SQL) |
|------|----------------|-----------------|
| Config (tst, tnt, tdt...) | Mat khi redeploy, re-seed | Persist, seed 1 lan |
| Tasks (tsi) | **MAT** khi redeploy | **PERSIST** vinh vien |
| Documents (tdi) | **MAT** metadata | **PERSIST** vinh vien |
| Events (tsev) | **MAT** audit log | **PERSIST** vinh vien |
| Assignments (tri) | **MAT** | **PERSIST** vinh vien |
| Files (GCS bucket) | OK (da persistent) | OK (khong thay doi) |

---

## 7.7 Cau hoi can lam ro (Open Questions)
...
5. **Contract tracking (LF246)**: Viec theo doi gia han HD co can tich hop lich (Calendar) khong? Hay chi gui email nhac?
...
| TST-051 | LF246 | Theo doi & nhac gia han | 2 | LF240 | ongoing |
| TST-052 | LF246.1 | Kiem tra HD sap het han | 3 | LF246 | monthly |
...
| TNT-040-21 | LF246.1 | LF246 | NULL | HD dang hieu luc |
| TNT-040-22 | LF246 | LF246.1 | NULL | Dinh ky |
| TNT-040-23 | LF246.1 | END | `expired_or_terminated` | Ket thuc |
| TNT-040-24 | LF246.1 | LF240 (new TSI) | `needs_renewal` | **Tao task moi** gia han |
...
| **Submitters** (MinhPT) | Upload Document + AI Check + Submit to Review | 3 nut: [Upload (tim)] [AI Check (indigo)] [Submit to Review (xanh)] |
...
|------|-------|------|-------|
| Trai (xanh duong) | **Submitters** | Product Manager | Submit legal review requests, upload documents, track progress |
| Phai (xanh la) | **Legal Admin** | Legal Manager | Review and approve submitted tasks, manage compliance |

Sidebar hien "Submitters" hoac "Legal Admin" thay vi ten thuc.
...
> v8.0 - SEC Permission System, Task Reassignment, GCS Storage, LF210/LF240 updates


---

# 10. Session Summary (3-4/4/2026)

## Deployments

| Service | Revision | URL |
|---------|----------|-----|
| Backend | lww-backend-00049-sjl | https://lww-backend-21672960606.asia-southeast1.run.app |
| Frontend | lww-frontend-00040-w87 | https://lww-frontend-21672960606.asia-southeast1.run.app |

## Infrastructure

| Component | Details |
|-----------|---------|
| Database | Cloud SQL PostgreSQL (legal-workflow-db, db-f1-micro, asia-southeast1) |
| File Storage | GCS bucket: legal-workflow-docs (ASIA-SOUTHEAST1) |
| Auth | BigQuery v_auth_lookup + EXCEPTION_USERS. USE_BIGQUERY_AUTH=true |
| Email | Gmail SMTP (trungnv1@apero.vn, App Password) |

## Bugs Fixed (P0)

| Bug | Root Cause | Fix |
|-----|-----------|-----|
| Login mat khi refresh | Token chi luu trong memory | localStorage persist + JWT expiry check |
| Uploaded By sai ma | uploaded_by luu emp_code | Doi sang emp_name tu JWT claims |
| Download khong hoat dong | fetch cross-origin bi block | <a download> tag thay vi fetch blob |
| Scroll nhay dau khi save | fetchTask() re-render toan trang | Luu/restore window.scrollY |
| Dashboard so lieu sai | Dem tat ca TSI (L1+L2+L3) | Chi dem L1 root + latest_status |
| Login 401 loop | API interceptor auto-logout on login 401 | Skip auto-logout cho /api/auth/login |
| MockPermissionService | USE_BIGQUERY_AUTH khong set | Them env var USE_BIGQUERY_AUTH=true |
| Step order lon xon | SELECT * FROM tsi khong co ORDER BY | ORDER BY created_at ASC |
| Submitter thay approve/reject | isEditable khong check isAdmin | isEditable = !isDone && allPrevDone && isAdmin |
| Submit to Review 403 | Submitter khong co trong TRI | Them check is_creator (requested_by) |
| Assigned To khong hien | Route /assignable bi match boi /{emp_code} | Dat /assignable truoc /{emp_code} |
| Page load 30s | N BigQuery queries cho name lookup | Batch 1 query + cache |

## Features Added (P1 + P2)

| Feature | Details |
|---------|---------|
| Dashboard clickable | Click status card -> filter My Tasks |
| Filter droplist | Datalist suggestions cho PT, LE, CDT, CTY, TUT, TLT |
| My Tasks UI | Header xanh, cot Assigned To + Submitted By (id_name) |
| Upload Cloud CTA | Cloud icon, border dashed, nut do |
| Email Notification | Send Back to Submitter (modal) + Notify Reviewer + Notify on Assign |
| Document Preview | Collapsible, select dropdown, PDF iframe / IMG preview (600px) |
| Upload PE | Nut Upload PE canh Ma PE, chi PDF/JPG/PNG |
| Create Task cleanup | Bo Department/Priority/Upload. Auto deadline T+1/T+2 |
| Assigned To BigQuery | Dropdown Checker + Approver tu BigQuery |
| Step order logic | Step chi editable khi tat ca step truoc da done |

## User Accounts

| Email | Role | Source |
|-------|------|--------|
| trungnv1@apero.vn | Approver (SEC4) | BigQuery |
| tiepta@apero.vn | Approver | BigQuery + Mock |
| thanhnc@apero.vn | SEC2 | BigQuery |
| legal@apero.vn | Submitters | EXCEPTION_USERS |
| nguyenvietrung187@gmail.com | Submitters | EXCEPTION_USERS |
| TrangCTH | Approver | BigQuery (updated 4/4) |

## Environment Variables (Cloud Run)

| Env Var | Value |
|---------|-------|
| DATABASE_URL | postgresql://legal_app:***@/legal_workflow?host=/cloudsql/fp-a-project:asia-southeast1:legal-workflow-db |
| GCS_BUCKET | legal-workflow-docs |
| STORAGE_BACKEND | gcs |
| USE_BIGQUERY_AUTH | true |
| SMTP_USER | trungnv1@apero.vn |
| SMTP_FROM | trungnv1@apero.vn |
